...
@objc public class Search: MetaZKit.Search {
               ...
               seasonBanners: [Int: [RemoteData]]) -> [[String: Any]]
    {
        return episodes.compactMap {(episode) in
            ...
            result[MZDirectorTagIdent] = episode.directors.join()
            result[MZScreenwriterTagIdent] = episode.writers.join()
            result[MZTVEpisodeIDTagIdent] = episode.productionCode
    ...
    override public func do_search() throws {
        ...
        for id in series {
            do {
                ...
                values[MZActorsTagIdent] = actors.map { $0.name }.join()
