...
@objc public class TVSearch : Search {
               ...
               seasonBanners: [RemoteData]) -> [[String: Any]]
    {
        return episodes.map {(episode) in
            ...
            result[MZTitleTagIdent] = episode.name
            //result[MZIMDBTagIdent] = episode.imdbId
            result[MZShortDescriptionTagIdent] = episode.overview
            result[MZLongDescriptionTagIdent] = episode.overview
            result[MZDirectorTagIdent] = episode.crew.filter { $0.department == "Directing" }.map{ $0.name }.join()
            result[MZScreenwriterTagIdent] = episode.crew.filter { $0.department == "Writing" }.map{ $0.name }.join()
            result[MZProducerTagIdent] =  episode.crew.filter { $0.department == "Production" }.map{ $0.name }.join()
            result[MZTVEpisodeIDTagIdent] = episode.production_code
    ...
    override public func do_search() throws {
        ...
        for id in series {
            do {
                ...
                values[MZTVNetworkTagIdent] = info.networks.map { $0.name }.join()
                values[MZShortDescriptionTagIdent] = info.overview
                ...
                if let credits = info.credits {
                    values[MZActorsTagIdent] = credits.cast.map { $0.name }.join()
                }
